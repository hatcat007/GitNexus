...
must_haves:
  truths:
    ...
    - "GitNexusError interface includes retryable: boolean and retryAfter?: number as top-level properties"
    - "All resilience error helpers set retryable appropriately (timeout/circuit/connection: true, retry_exhausted: false)"
  artifacts:
    - path: "gitnexus-mcp/src/mcp/errors.ts"
      provides: "Extended error codes for resilience failures with retry guidance"
      contains: "retryable: boolean | retryAfter"
...
<task type="auto">
  <name>Task 2: Add resilience error helper functions with retry guidance</name>
  <files>gitnexus-mcp/src/mcp/errors.ts</files>
  <action>
First, update the `GitNexusError` interface to include retry guidance as top-level properties:
...
Then add helper functions for creating resilience errors at the end of errors.ts:
...
IMPORTANT: `retryable` and `retryAfter` are TOP-LEVEL properties on GitNexusError, not nested in `details`. This matches the locked decision in CONTEXT.md: "Always include retry guidance: retryable: boolean, retryAfter?: number".

All errors should include actionable suggestions. Debug info only included when GITNEXUS_DEBUG=true.
  </action>
  <verify>grep -E "export function (timeoutError|circuitOpenError|connectionLostError|retryExhaustedError)" gitnexus-mcp/src/mcp/errors.ts && grep -E "retryable:" gitnexus-mcp/src/mcp/errors.ts | head -10</verify>
  <done>Four new error helper functions exported with retryable field: timeoutError (retryable: true), circuitOpenError (retryable: true, retryAfter: 30), connectionLostError (retryable: true), retryExhaustedError (retryable: false)</done>
</task>
