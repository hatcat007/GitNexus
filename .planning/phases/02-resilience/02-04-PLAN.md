...
must_haves:
  truths:
    ...
    - "Server is passive - backoff is for diagnostics/state tracking, browser initiates actual reconnection"
  artifacts:
...
**Note on reconnection strategy:** The WebSocket server is PASSIVE - it accepts connections from the browser. When the browser disconnects, we cannot force it to reconnect. The exponential backoff here serves two purposes:
1. **Diagnostics**: Log when reconnection is expected with backoff timing, helping debug connection issues
2. **State tracking**: Maintain attempt counter for error reporting (e.g., "reconnection attempt 3")
The actual reconnection is initiated by the browser-side code (not in this file).
...
When context arrives (browser reconnects), we reset the counter.
  ...
  <done>Browser disconnect triggers reconnection scheduling with exponential backoff. Purpose clarified: diagnostics + state tracking (server is passive, browser initiates reconnect).</done>
</task>
...
- Exponential backoff used for reconnection (500ms → 60s cap)
- Attempt counter resets on successful connection
